    ...
    getAsync(options) {
        ...
        return (() => {
            ...
            for (let i in this._data) {
                ...
                promises.push(options.xhr(this._type, src)
                    .then(xhr => {
...
class LoaderBase {
    ...
    _resolveXhrOptions(xhr) {
        if (!xhr) {
            return {};
        }
        const requestOptions = typeof (xhr) === 'boolean' ? {} : (xhr.requestOptions || {});
        return {
            requestOptions
        };
    }
    ...
    _loadAsync(target, options = {}) {
        ...
        return new Promise(resolve => {
            ...
            if (bt.valid) {
                ...
                resolve(new TextureLoaderResource(new Texture(bt), null));
                return;
            }
            bt.on('loaded', (baseTexture) => {
                BaseTexture.removeFromCache(baseTexture);
                resolve(new TextureLoaderResource(new Texture(baseTexture), null));
            });
            bt.on('error', (baseTexture, e) => {
                BaseTexture.removeFromCache(baseTexture);
                resolve(new TextureLoaderResource(new Texture(baseTexture), e));
            });
        });
    }
...
class SpritesheetLoader extends LoaderBase {
    ...
    _loadTextureFromJson(json, options) {
 ...
 */
const _manifests = {};
class Content extends Emitter$1 {
    ...
    _loadAssetAsync(manifests) {
        const basepaths = this._piximData.basepath;
...
export { Application, Container, Content, ContentDeliver, EVENT_LOADER_ASSET_LOADED, Emitter, JsLoader, JsLoaderResource, JsonLoader, JsonLoaderResource, JsonManifest, Layer, LoaderBase, LoaderResource, ManifestBase, SoundLoader, SoundLoaderResource, SoundManifest, SpritesheetLoader, SpritesheetLoaderResource, SpritesheetManifest, Task$1 as Task, TextureLoader, TextureLoaderResource, TextureManifest, index as utils };
